...
const SeoRecord = ({primary, secondary}: {primary: string, secondary: any}) => {
  return (
    <Stack alignContent={"center"} alignItems={"center"}>
      ...
      <Typography variant={"subtitle1"} color={"#e94235ff"} ><ValueOrProgress value={secondary}/></Typography>
    </Stack>
  )
}
export default function App() {
  ...
  useEffect(()=> {
    (async () => {
      ...
      // @ts-ignore
      chrome.tabs.sendMessage<ReportActionKind, ContentResponse>(tab.id, Command.GET_CONTENT).then((response) => {
  ...
  const Report = () => {
    if (!!err) {
      return <Alert severity={"error"}>PageDigest is unavailable: <b>{err}</b></Alert>
    }
    return (
      <Grid container>
        <Grid item xs={4}>
